...
function Header(props) {
  ...
  const handleConfirmLogout = useCallback(() => {
    confirm({
      content: "Bạn có muốn đăng nhập lại?",
      titleSubmitBtn: "Đăng xuất",
    }).subscribe((answer) => {
      if (!answer) return;
  ...
  const onChangePass = (values) => {
    const changePassSub = changePass(values).subscribe(
      (res) => {
  ...
  function ModalChangePass(props: ModalProps) {
    return (
      <Form layout="vertical" onFinish={onChangePass} form={form}>
        <div className="modal-body p-3">
          <div className="modal-title de-mb-2 modal-title-2">Đổi mật khẩu</div>
          ...
          <div className="modal-actions">
            ...
            <Button
              onClick={() => props.modalRef.close(form.getFieldsValue())}
              type="primary"
  ...
  const openModalChangePass = () => {
    const modalChange = openModal(ModalChangePass, {
      dialogClassName: "de-modal-md modal-custom",
      closeButton: true,
    });
